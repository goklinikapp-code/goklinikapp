...
class _AppointmentDetailSheetState extends State<AppointmentDetailSheet> {
  ...
  Widget build(BuildContext context) {
    ...
    final canStart = widget.appointment.status == 'confirmed';
    final canComplete = widget.appointment.status == 'in_progress';
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                  ...
                  child: Column(
                    children: [
                      Row(
                        children: [
                          GKAvatar(
                            name: widget.appointment.patientName.isEmpty
                                ? widget.t('patient_default')
                                : widget.appointment.patientName,
                            imageUrl:
                                widget.appointment.patientAvatarUrl.isEmpty
                                    ? null
                                    : widget.appointment.patientAvatarUrl,
                          ),
                          const SizedBox(width: 10),
                          Expanded(
                            child: Text(
                              widget.appointment.patientName.isEmpty
                                  ? widget.t('patient_default')
                                  : widget.appointment.patientName,
                      ...
                      _InfoRow(
                        ...
                        label: 'Profissional',
                        value:
                            widget.appointment.professionalName.trim().isEmpty
                                ? widget.t('role_surgeon')
                                : widget.appointment.professionalName.trim(),
                      ),
                      _InfoRow(
                ...
                if (widget.appointment.cancellationReason
                    .trim()
                    .isNotEmpty) ...[
                  const SizedBox(height: 12),
                  GKCard(
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(widget.appointment.cancellationReason.trim()),
                      ],
                    ),
                  ),
                ],
                    ...
                    children: [
                      if (canStart)
                        FilledButton.icon(
                          onPressed: _loading
                              ? null
                              : () => _submitStatus('in_progress'),
                          style: FilledButton.styleFrom(
                            backgroundColor: const Color(0xFFF59E0B),
                          ),
                          icon: const Icon(Icons.play_circle_fill),
                          label: Text(
                              _loading ? 'Salvando...' : 'Iniciar atendimento'),
                        ),
                      if (canComplete)
                        FilledButton.icon(
                          onPressed: _loading
                              ? null
                              : () => _submitStatus('completed'),
                          style: FilledButton.styleFrom(
  ...
  Future<void> _submitStatus(
    ...
    try {
      ...
      if (!mounted) return;
      Navigator.of(context).pop(_successMessage(status));
    } catch (error) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(
            _extractApiError(
              error,
              fallback: 'Não foi possível atualizar o status do agendamento.',
            ),
          ),
        ),
      );
